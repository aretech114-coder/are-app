    ...
    <div className="space-y-3">
      ...
      {showStep6 && (
        <FieldCategory title={UI_LABELS.dgValidation} displayStep={visual(6)} color="rose">
          {workflowCtx.loading ? (
            <p className="text-xs text-muted-foreground">Chargement...</p>
          ) : workflowCtx.ministerValidation ? (
            <StepNote text={workflowCtx.ministerValidation} iconColor="text-rose-600" />
          ) : (
      ...
      {shouldShowWorkflowBlock(8, currentStep, activeSteps) && (
        <FieldCategory title="Preuve de dépôt" displayStep={visual(8)} color="indigo">
          <p className="text-xs text-muted-foreground">Secrétariat — preuve de dépôt / retour.</p>
        </FieldCategory>
      )}
