...
import { LoginPageShell, useLoginPageAppearance } from "@/components/LoginPageShell";
      ...
      <Card className={cardGlassClass}>
        <CardHeader className="text-center items-center">
          ...
          {settings.show_login_title !== "false" && (
            <>
              <h1 className={`text-xl font-bold ${titleOnBgClass}`}>
                {settings.site_title || "ARE App"}
              </h1>
              <p className={`text-xs -mt-1 ${subtitleOnBgClass}`}>
                Système de Gestion Électronique du Courrier
              </p>
              <div className={`w-12 h-px my-2 ${dividerClass}`} />
            </>
          )}
          <CardTitle className={`text-lg ${cardTitleClass}`}>Connexion</CardTitle>
