...
BEGIN
  ...
  -- Conditions skip / fallback
  LOOP
    SELECT ws.id, ws.conditions INTO v_target_step_id, v_step_conditions
    FROM workflow_steps ws
    ...
    -- Ministre absent : tenter fallback sinon skip
    IF (v_step_conditions->>'skip_if_ministre_absent')::boolean IS TRUE AND v_ministre_absent THEN
      v_fallback_user := resolve_fallback_user(v_target_step_id, 'ministre_absent');
      IF v_fallback_user IS NOT NULL THEN
        v_fallback_used := 'ministre_absent';
        EXIT;
      END IF;

      INSERT INTO workflow_transitions (mail_id, from_step, to_step, action, performed_by, notes)
      VALUES (_mail_id, v_current_step, v_new_step, 'skip', _performed_by, 'Étape ignorée — Ministre absent (aucun suppléant disponible).');
  ...
  INSERT INTO workflow_transitions (mail_id, from_step, to_step, action, performed_by, notes)
  VALUES (_mail_id, v_current_step, v_new_step, _action, _performed_by,
    CASE WHEN v_fallback_used IS NOT NULL
         THEN COALESCE(_notes, '') || ' [Fallback déclenché: ' || v_fallback_used || ']'
         ELSE _notes END);
  ...
  ELSE
    -- Si fallback déclenché => prioriser ce fallback user
    IF v_fallback_user IS NOT NULL THEN
      INSERT INTO mail_assignments (mail_id, assigned_by, assigned_to, step_number, status, instructions)
      VALUES (_mail_id, _performed_by, v_fallback_user, v_new_step, 'pending',
              'Assignation suite à fallback (' || v_fallback_used || ')');
      INSERT INTO notifications (user_id, title, message, mail_id)
      VALUES (v_fallback_user, 'Courrier en attente (suppléant)', 'Vous êtes désigné comme suppléant pour traiter ce courrier.', _mail_id);
      v_resolved_assignee := v_fallback_user;
    ELSIF _assignee_ids IS NOT NULL AND array_length(_assignee_ids, 1) > 0 THEN
      INSERT INTO mail_assignments (mail_id, assigned_by, assigned_to, step_number, status)
