-- Doublons mail_assignments à l'étape 4 :
-- promote proposed→pending puis INSERT des mêmes assignés si _assignee_ids est passé
-- → 2 lignes par personne (compteur x2, notifications en double).

-- 1) Supprimer les doublons (garder la ligne la plus avancée / la plus ancienne)
DELETE FROM public.mail_assignments
WHERE id IN (
  SELECT id
  FROM (
    SELECT
...
-- 2) Empêcher les futurs doublons
CREATE UNIQUE INDEX IF NOT EXISTS mail_assignments_mail_user_step_mode_key
  ON public.mail_assignments (mail_id, assigned_to, step_number, access_mode);

-- 3) advance_workflow_step — entrée étape 4 sans double INSERT
...
BEGIN
  ...
  IF v_new_step = 4 THEN
    IF _assignee_ids IS NOT NULL AND array_length(_assignee_ids, 1) > 0 THEN
      -- Liste explicite : remplacer tous les contributors (évite doublon après promote proposed→pending)
      DELETE FROM mail_assignments
      WHERE mail_id = _mail_id AND step_number = 4 AND access_mode = 'contributor';
      INSERT INTO mail_assignments (mail_id, assigned_by, assigned_to, step_number, status, access_mode)
